...
def distribute_clusters(
    ...
    # Workaround for Windows / appsi_highs:
    # Avoid solving the quadratic cluster-allocation problem with Pyomo.
    # Instead, allocate clusters proportionally using largest-remainder rounding.

    target = n_clusters * factors / factors.sum()

    lower = pd.Series(
        {r: int(m.n[r].lb if m.n[r].lb is not None else 1) for r in factors.index},
        index=factors.index,
    )

    upper = pd.Series(
        {r: int(m.n[r].ub if m.n[r].ub is not None else n_clusters) for r in factors.index},
        index=factors.index,
    )

    allocation = np.floor(target).astype(int)
    allocation = allocation.clip(lower=lower)
    allocation = pd.concat([allocation, upper], axis=1).min(axis=1).astype(int)

    remainder = target - allocation

    while allocation.sum() < n_clusters:
        candidates = allocation[allocation < upper].index
        if len(candidates) == 0:
            break
        r = remainder.loc[candidates].idxmax()
        allocation.loc[r] += 1
        remainder.loc[r] -= 1

    while allocation.sum() > n_clusters:
        candidates = allocation[allocation > lower].index
        if len(candidates) == 0:
            break
        r = remainder.loc[candidates].idxmin()
        allocation.loc[r] -= 1
        remainder.loc[r] += 1

    assert allocation.sum() == n_clusters, (
        f"Cluster allocation failed: allocated {allocation.sum()} but expected {n_clusters}"
    )
